...
import { getAvailablePDFs, extractTextFromPDF, createVectorStore, queryVectorStore } from './rag.js';
...
async function testRAG() {
  ...
  try {
    ...
    const text = await extractTextFromPDF(testPDF.path);
    const duration = ((Date.now() - startTime) / 1000).toFixed(2);
    
    console.log(`   ✓ Successfully extracted ${text.length} characters in ${duration}s`);
  ...
  try {
    const startTime = Date.now();
    const vectorStore = await createVectorStore(testPDF.name, text.substring(0, 5000)); // Use first 5000 chars for testing
    const duration = ((Date.now() - startTime) / 1000).toFixed(2);
